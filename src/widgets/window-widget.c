...
static twtk_widget_t *_create(twtk_rect_t rect, twtk_widget_t* client)
{
    if (client == NULL)
        client = _create_subframe();

    if (client->name == NULL)
        twtk_widget_set_name(client, "sub-0");
    ...
    priv->client_area = twtk_widget_ref(client);

    twtk_widget_set_frame(client, window);

    /* create the close button */
    priv->close_button = twtk_button_widget_create(CLOSEBTN_IMAGE, twtk_rect_null(), "window-close");
    assert(priv->close_button);
...
twtk_widget_t *twtk_window_widget_create(twtk_widget_t *client)
{
    assert(client);
    twtk_widget_t *window = _create(calc_window_geometry(client->viewport), client);
    return window;
}
